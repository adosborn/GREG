...
let rec draw_function (funct: float -> float, domain: Domain, cur_value: float, color: string, line: string, iter: int) : string = 
    ...
    let is_dash = ((iter % int(24.0/(RESOLUTION*2.0))) - int(12.0/(RESOLUTION*2.0))) >= 0
    let is_dot = (iter % int(16.0/(RESOLUTION*2.0))) = 0

    match line, over_max, is_dash, is_dot with 
    | _, true, _, _  -> ""
    | "Solid", false, _, _ ->       "<line x1=\"" + (starting_point_x_cc |> string) + "\" x2=\"" + (ending_point_x_cc |> string) + 
                                    "\" y1=\"" + (starting_point_y_cc |> string) + "\" y2=\"" + (ending_point_y_cc |> string) + 
                                    "\" stroke=\"" + color + "\" stroke-width=\"5\"/>\n" + 
                                    draw_function (funct, domain, cur_value + (RESOLUTION/scaling_factor), color, line, iter+1)
    | "Dashed", false, true, _ ->   "<line x1=\"" + (starting_point_x_cc |> string) + "\" x2=\"" + (ending_point_x_cc |> string) + 
                                    "\" y1=\"" + (starting_point_y_cc |> string) + "\" y2=\"" + (ending_point_y_cc |> string) + 
                                    "\" stroke=\"" + color + "\" stroke-width=\"5\"/>\n" + 
                                    draw_function (funct, domain, cur_value + (RESOLUTION/scaling_factor), color, line, iter+1)
    | "Dashed", false, _, _ ->      draw_function (funct, domain, cur_value + (RESOLUTION/scaling_factor), color, line, iter+1)
    | "Dotted", false, _, true ->   "<circle cx=\"" + (starting_point_x_cc |> string) + "\" cy=\"" + (starting_point_y_cc |> string) + "\" r=\"3\" 
                                    fill=\"" + color + "\"/>\n" + 
                                    draw_function (funct, domain, cur_value + (RESOLUTION/scaling_factor), color, line, iter+1)
